...
1.功能切换时，比如文生图切换成图生图的时候，不管下面有什么红色的报错信息，应该都要重置，现在会产生报错后，切换功能报错信息一直显示的问题
2.边缘检测，深度图模式选择后，是不是应该第一时间调用对应的模型（如果找不到模型则应该报错提示），自动预处理，将预处理后的图片渲染替换到上面的原图的预览框中直接作为参数
3.web界面的pose模式没有实现，补上
4.高级模式中是否应该加上seed,num_images的参数，这两个参数你需要搜索官方文档，是每个模式都支持的吗，是每个模式都可选的吗，调研完更新api文档
5.局部重绘应该默认是auto_mask模式，并自动调用对应的模型生成mask，然后放到上面的原图预览的右边做对比，支持画笔手绘微调，只要微调，是不是就应该将auto_mask改为false，可以重置微调，重置后auto_mash恢复为true,然后auto_mask是你自己加的参数还是官方的，是否可以去掉，如果不去掉，这个auto_mask和prompt的优先级是怎么样的，是会覆盖还是合并，搜索官方文档调研
6.结合官方文档，调研是否可以用ltx-2.3-22b-distilled-lora-384.safetensors替换ltx-2-19b-distilled-lora-384.safetensors，5090 32g的显卡0会参数不兼容或者爆显存吗，效果会更好吗，还是不支持替换