...
5.局部重绘应该默认是auto_mask模式，并自动调用对应的模型生成mask，然后放到上面的原图预览的右边做对比，支持画笔手绘微调，只要微调，是不是就应该将auto_mask改为false，然后auto_mask是你自己加的参数还是官方的，是否可以去掉，如果不去掉，这个auto_mask和prompt的优先级是怎么样的，是会覆盖还是合并，搜索官方文档调研